...
def main():
    ...
    # (bicycle motorcycle) (dog cat) (bird plane) (einstein marilyn) (fish submarine) (horse, zebra)
    main_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    image1 = cv2.cvtColor(cv2.imread(os.path.join(main_path, 'data', 'cat.bmp')), cv2.COLOR_BGR2RGB)
    image2 = cv2.cvtColor(cv2.imread(os.path.join(main_path, 'data', 'dog.bmp')), cv2.COLOR_BGR2RGB)
    ...
    # print(np.max(hybrid_image),np.min(hybrid_image))
